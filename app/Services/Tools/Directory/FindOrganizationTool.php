...
final class FindOrganizationTool implements JarvisTool
{
    public const NAME = 'find_organization';
    ...
    public function definition(): ToolDefinition
    {
        return new ToolDefinition(
            name: self::NAME,
            description: 'Find canonical Organizations by name.',
            parameters: [
                'type' => 'OBJECT',
                'properties' => [
                    'query' => ['type' => 'STRING'],
    ...
    public function execute(ToolCall $call, ToolExecutionContext $context): ToolResult
    {
        ...
        try {
            $organizations = $this->directory->findOrganizations($context->user, $query);
        ...
        return ToolResult::success($call->id, $this->name(), [
            'success' => true,
            'organizations' => $organizations->map(fn ($organization): array => $this->directory->serializeOrganization($organization))->values()->all(),
        ]);
    }
}
